...
if (/^node /.test(s.statusLine.command)) throw new Error('command uses bare "node" instead of an absolute, quoted path');
const nodePathPosix = process.execPath.replace(/\\/g, '/');
if (s.statusLine.command.indexOf('"' + nodePathPosix + '"') === -1) throw new Error('command does not embed the running node executable, quoted');
